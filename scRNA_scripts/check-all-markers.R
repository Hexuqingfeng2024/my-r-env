...
if(sp=='human'){
   lapply(markers, function(x){
     ...
     ggsave(paste('check_for_',x,'.pdf'),width  = 18,height = 9)
   })
  ...
  last_markers_to_check <<- str_to_upper(last_markers ) 
   ...
   lapply(markers_list, function(x){
     ...
     w=length( unique(unlist(genes_to_check)) )/5+6;w
     ggsave(paste('check_for_',x,'.pdf'),width  = 18,height = 8)
   })
...
ggsave(paste('last_markers_and_umap.pdf'),width  = 18,height = 8)
...
if("percent_mito" %in% colnames(sce.all.int@meta.data ) ){
  ...
  ggsave(filename=paste0(pro,"Vlnplot1.pdf"),plot=p1,width  = 18,height = 8)
  ...
  ggsave(filename=paste0(pro,"Vlnplot2.pdf"),plot=p2,width  = 18,height = 8)
...
if(T){
  ...
  ggsave(paste0(pro,'DotPlot_check_top3_markers_by_clusters.pdf'),width=18,height=8)
